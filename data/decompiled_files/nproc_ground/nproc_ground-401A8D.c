...
__int64 __fastcall sub_401A8D(int a1)
{
  __int64 result; // rax
  ...
  if ( a1 == 2 )
  {
    ...
    if ( nptr )
    {
      while ( *nptr && (unsigned __int8)sub_404660((unsigned int)*nptr) )
        ++nptr;
      if ( (unsigned __int8)sub_40458A((unsigned int)*nptr) )
      {
        endptr = 0LL;
        v6 = strtoul(nptr, &endptr, 10);
        if ( endptr )
        {
          while ( *endptr && (unsigned __int8)sub_404660((unsigned int)*endptr) )
            ++endptr;
          if ( !*endptr )
          {
            if ( v6 )
