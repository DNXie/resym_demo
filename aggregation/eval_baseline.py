    ...
    def _eval_group(group_data):
        ...
        for funvar, funvardata in group_data['votes'].items():
            ...
            if 'stack' in funvardata and not is_pred_ptr(funvardata['stack'][1]):
                ...
                struct_type_evaluator.update(gt_type, funvardata['stack'][1])
                stack_pred_layout = funvardata.get('stack_offsets', [])
                stack_pred_layout = {k:v for (k,v) in stack_pred_layout}
                offset_evaluator.update(stack_offsets, stack_pred_layout)
                for offset in stack_layout:
                ...
                if 'heap' in funvardata:
                    stack_pred_type = list(funvardata['heap'].values())[0]['pred'][1]
                elif 'stack' in funvardata:
                    stack_pred_type = funvardata['stack'][1]
                else:
                    stack_pred_type = "-"
                struct_type_evaluator.update(gt_type, strip_type(stack_pred_type))
                heap_pred_offsets = funvardata.get('heap_offsets', [])
                heap_pred_offsets = {k:v for (k,v) in heap_pred_offsets}
                offset_evaluator.update(heap_offsets, heap_pred_offsets)
                for offset in heap_layout:
                    if str(offset) in funvardata.get('heap', {}):
                        pred_name = funvardata['heap'][str(offset)]['pred'][2]
                        pred_type = funvardata['heap'][str(offset)]['pred'][3]
                    else:
                        pred_name = '-'
                        pred_type = '-'
