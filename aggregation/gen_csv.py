...
def get_gt_nametype(funvardata) -> (str, str):
    try:
        return funvardata['stack_gt']
    except:
        return None, None
...
def main(prep_dir, equiv_vars_dir, prep_eval_folder, save_fpath):
    csv_file = [['binary_prog', 'funname', 'funaddr', 'var', 'varname', 'type', 'result_layout', 'result_ptr', 'iter']]
    for binfile in get_file_list(prep_dir):
        ...
        for group_f in get_file_list(bin_prep_eval_folder):
            ...
            for funvar, funvardata in group_data['votes'].items():
                ...
                varname_gt, vartype_gt = get_gt_nametype(funvardata)
                if varname_gt is None: 
                    # assert False, f"{os.path.join(bin_prep_eval_folder, group_f)}   {fun}, {var}"
                    print(f"{os.path.join(bin_prep_eval_folder, group_f)}   {fun}, {var}")
                    continue
                # if var_is_ptr:
                csv_file.append([
                    binname_stripped, funvardata['funname'], fun, var, varname_gt, vartype_gt, 
        ...
        for fun, fundata in prepdata.items():
            ...
            if 'heap' in fundata:
                ...
                for var in heap_offsets:
                    ...
                    seen_funvar.add(f'{fun}---{var}')
                    if 'stack' not in fundata:
                        continue
                    varname_gt, vartype_gt = fundata['stack']['ground_truth'][var]
                    csv_file.append([
                        binname_stripped, fundata['funname'], fun, var, varname_gt, vartype_gt, 
            ...
            if 'stack' in fundata:
                ...
                for cp in cluster_pred:
                    ...
                    varname_gt, vartype_gt = fundata['stack']['ground_truth'][head]
                    offsets = get_cluster_offset(fundata, cp)
                    csv_file.append([
                        binname_stripped, fundata['funname'], fun, head, varname_gt, vartype_gt, 
